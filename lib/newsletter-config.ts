...
export function newsletterConfigured(): boolean {
  const present = process.env.NEWSLETTER_ENABLED === "true" && [
    "RESEND_API_KEY", "RESEND_SEGMENT_ID", "NEWSLETTER_FROM", "NEWSLETTER_CONTACT_EMAIL",
    "NEWSLETTER_OWNER", "UPSTASH_REDIS_REST_URL", "UPSTASH_REDIS_REST_TOKEN", "CRON_SECRET",
  ].every(key => !!process.env[key]?.trim());
  if (!present || process.env.CRON_SECRET!.length < 32 || !normalizeEmail(process.env.NEWSLETTER_CONTACT_EMAIL)) return false;
  try {
...
export const NEWSLETTER_CONSENT_VERSION = "2026-09-19";
